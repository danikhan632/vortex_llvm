...
static constexpr StringLiteral kTilesInUseAttr("vortex_gpu.tiles_in_use");
static constexpr StringLiteral
    kNextInMemoryTileIdAttr("vortex_gpu.next_in_memory_tile_id");
...
static ArrayRef<TileMask> getMasks(VortexGPUTileType type) {
  ...
  switch (type) {
  case VortexGPUTileType::ZAB:
    return ZA_B_MASKS;
  case VortexGPUTileType::ZAH:
    return ZA_H_MASKS;
  case VortexGPUTileType::ZAS:
    return ZA_S_MASKS;
  case VortexGPUTileType::ZAD:
    return ZA_D_MASKS;
  case VortexGPUTileType::ZAQ:
    return ZA_Q_MASKS;
  }
}
...
/// left.
static FailureOr<unsigned> allocateTileId(VortexGPUTileType tileType,
                                          TileMask &tilesInUse) {
...
struct AssignTileIDsPattern
    : public OpInterfaceRewritePattern<VortexGPUTileOpInterface> {
  using OpInterfaceRewritePattern::OpInterfaceRewritePattern;
  LogicalResult matchAndRewrite(VortexGPUTileOpInterface tileOp,
    ...
    std::optional<VortexGPUTileType> tileType = tileOp.getAllocatedTileType();
    if (!tileType)
    ...
    for (auto *op : dependantOps) {
      if (auto dependantTileOp = llvm::dyn_cast<VortexGPUTileOpInterface>(op)) {
...
struct TileAllocationPass
    : public vortex_gpu::impl::TileAllocationBase<TileAllocationPass> {
